...
How to troubleshoot in OpenShift using Red Hat build of OpenTelemetry.  Add the OpenTelemetry components (sidecar, instrumentation) to your project, so that the application will send the tracing data to OpenTelemetry, then determine the possible cause of a 500 error that is being generated by the app and take the necessary steps to resolve the issue.
